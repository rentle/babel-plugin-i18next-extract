...
function resolveIfRelative(path$1) {
...
function isCustomImportedNode(absoluteNodePaths, path$1, name) {
  return absoluteNodePaths.some(([sourceModule, importName]) => {
    if (path.isAbsolute(sourceModule)) {
      let relativeSourceModulePath = path.relative(path.dirname(path$1.state.filename), sourceModule);

      if (!relativeSourceModulePath.startsWith('.')) {
        relativeSourceModulePath = '.' + path.sep + relativeSourceModulePath;
